...
(function () {
    ...
    _app.prototype = {
        ...
        get_isTagged: function (tag) {
            ...
            return tagList && Array.contains(tagList, tag);
        },
        ...
        _showImport: function (view) {
            ...
            sb.appendFormat('<div class="Buttons"><button id="{0}$StartImport" onclick="$find(\'{0}\')._startImportProcessing();return false" style="display:none">{1}</button><button id="{0}$CancelImport" onclick="$find(\'{0}\')._closeImport();return false">{2}</button></div>', this.get_id(), resourcesData.Import.StartButton, Web.DataViewResources.ModalPopup.CancelButton);
        ...
        _startImportProcessing: function () {
            ...
            if (String.isBlank(email) && !confirm(resourcesData.Import.EmailNotSpecified)) {
                //emailElem.focus();
                ...
                if (currentFilter.length > 0 && !this.filterIsExternal()) {
                    for (var i = 0; i < currentFilter.length; i++) {
                        ...
                        if (!filter[2].startsWith('~')) {
                            ...
                            var fd = $app.filterDef(resourcesDataFilters[field.FilterType].List, field.FilterType == 'Boolean' && m[3].length > 1 ? (m[3] == '%js%true' ? '$true' : '$false') : func);
                            if (fd) {
                                var m2 = v.match(/^(.+?)\$and\$(.+?)$/);
            ...
            for (i = 0; i < currentFilter.length; i++) {
                ...
                while (m && (m[1].startsWith('~') || !(field.Index == field.AliasIndex && field.IsPrimaryKey && field.Hidden))) {
                    if (!first)
                        ...
                        sb.appendFormat('<span class="FilterElement" onclick="$find(\'{0}\').removeFromFilterByIndex({1})" title="{2}">', this.get_id(), isQuickFind ? -1 : field.Index, isQuickFind ? resourcesDataFiltersLabels.Clear : _app.htmlAttributeEncode(String.format(Web.DataViewResources.HeaderFilter.ClearFilter, field.Label)));
                    ...
                    if (m[1].startsWith('~')) {
                        ...
                        var fd = $app.filterDef(resourcesDataFilters[field.FilterType].List, field.FilterType == 'Boolean' && m[3].length > 1 ? (m[3] == '%js%true' ? '$true' : '$false') : m[1]);
                        if (!fd) {
                            switch (m[1]) {
                                case '=':
        ...
        _findItemByValue: function (field, value) {
            ...
            return field.ContextFields ? [value, value] : (itemCache[value] || [null, this.get_isForm() || _app.touch/* this.get_viewType() == 'Form'*/ ? resourcesData.NullValueInForms : resourcesData.NullValue]);
        },
        ...
        _invoke: function (methodName, params, onSuccess, userContext, onFailure) {
            ...
            function retry(error, response, method) {
                ...
                if (statusCode == 0) {
                    $app.confirm(resourcesData.ConnectionLost,
                        function () {
