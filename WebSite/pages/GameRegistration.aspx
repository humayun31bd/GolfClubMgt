...
<%--                                    <tr>
                                    ...
                                    <tr colspan="2">
                                        <td class="all-uppercase vam text-left">Need Golf Cart: <input type="checkbox" ng-true-value="1"  ng-false-value="0" name="needcart" ng-model="needgcart"  ng-disabled ="readonlyneedcart" ng-click="GolfCartFeeNeed()"></td>
                                        <td class="all-uppercase vam text-left">
        ...
        .controller('GameRegistrationCtrl', function ($scope, $http, $uibModal, $log, $document, $templateCache) {
            ...
            $scope.open = function (b, regDate, regTime, fSchID, fSchno, starttime, ssUserName, pNumberOfPlayer, pMaxMemberCount) {
                ...
                function ModalInstanceCtrl($scope, $uibModalInstance, tId, $http, rDate, rTime, fSchID, starttime, fSchno, ssUserName) {
                    ...
                    $scope.gcartFee = function () {
                        ...
                        $http.get('http://localhost:2997/api/Game/GetGolfCartGameFee?pMemberCode=' + memid +
                    ...
                    $scope.parmanantCadii = function () {
                        ...
                        $http.get('http://localhost:2997/api/Game/MemberPersonalCaddieBallboy?pMemberCode=' + memid + '').then(
                            function (response) {
                    ...
                    $scope.payNow = function () {
                        ...
                        $http({
                            method: "POST",
                            url: "http://localhost:2997/api/Game/CreateMemberGamePayNew",
                            data: obj
                            ...
                            console.log(response.data.RegDate);

                            var pdate = response.data.RegDate;
                            /*var pdate = response.data.RegDate ? format(response.data.RegDate, 'MM/dd/yyyy').toString() : format(new Date(), 'MM/dd/yyyy').toString();*/
                            $http.get('http://localhost:2997/api/Game/GameFlightSchedules?pGameDate=' + "'" + pdate + "'" +
                                '&pGameDate1=' + "'" + pdate + ' 23:59:59' + "'").then(function (response) {
                                    if (response.data) {
                                        $scope.schyList = response.data;
                                    ...
                                    console.log('MemberBillID', MemberBillID);
            ...
            $scope.show = function (b, regDate, regTime, fSchID, fSchno, starttime, mId, p, ssUserName) {
                ...
                function ModalInstanceCtrl($scope, $uibModalInstance, tId, $http, rDate, rTime, fSchID, starttime, fSchno, mId, p) {
                    ...
                    $scope.Preball8 = p.m_GolfCartFee;

                    $scope.MemberName = p.m_MemberName;
                    $scope.HandiCap = p.m_HandiCap;
                    $scope.MemberStatusName = p.m_MemberStatus;
                    $scope.fSchID = fSchID;
                    $scope.fSchNo = fSchno;
                    /*$scope.playDate = rDate;*/
                    $scope.estime = starttime;
                    ...
                    $scope.ball8 = p.m_GolfCartHoleTypeID ? p.m_GolfCartHoleTypeID : 0
                    $scope.needgcartID = p.m_GolfCartID

                    console.log($scope.needgcart, p.m_NeedGolfCart, p)

                    if (p.m_NeedGolfCart) {
                        $scope.readonlyneedcart = true;

                    } else {
                        $scope.readonlyneedcart = false;
                    }
                    ...
                    $scope.calculateTotalAmount = function (cartFee, ineedgcart) {
                        ...
                        if ($scope.fee.m_GreenFee) {
                            $scope.grandTotal += $scope.fee.m_GreenFee;
                            console.log('fee green fee Added', $scope.fee.m_GreenFee);
                        }
                        else {
                            $scope.grandTotal += 0;
                        }
                        ...
                        if ($scope.bboyinfo) {
                            ...
                            if ($scope.bboyinfo > 0) {
                                $scope.grandTotal += $scope.fee.m_BallBoyFee;
                                $scope.grandTotal -= $scope.fee.m_BallBoySubsidy;
                                console.log('ball boy fee Added', $scope.fee.m_BallBoyFee);
                                console.log('ball boy fee subs', $scope.fee.m_BallBoySubsidy);
                            } else {
                                $scope.grandTotal += 0;
                                $scope.bboyinfo = false;
                        ...
                        if ($scope.cadeinfo) {
                            if ($scope.cadeinfo > 0) {
                                $scope.grandTotal += $scope.fee.m_CaddieFee;
                                $scope.grandTotal -= $scope.fee.m_CaddieSubsidy;
                                console.log('caddie fee Added', $scope.fee.m_CaddieFee);
                                console.log('caddie fee subs', $scope.fee.m_CaddieSubsidy);
                            } else {
                                $scope.grandTotal += 0;
                                $scope.cadeinfo = false;
                        ...
                        if ($scope.previousfee.grandTotal > 0) {
                            $scope.grandTotal -= $scope.previousfee.grandTotal;
                            console.log('previous fee Added', $scope.previousfee.grandTotal);
                        } else {
                            $scope.grandTotal += 0;
                            $scope.cadeinfo = false;
                    ...
                    $scope.gamefee = function (hole) {
                        ...
                        console.log(hole, "HHHH", 'http://localhost:2997/api/game/GetGameFee?pMemberCode=' + memid + '&pHoleTypeID=' + hole + '&pGameDate=' + format($scope.regDate, 'MM/dd/yyyy').toString());
                        $scope.memberfee = {};


                        var url = 'http://localhost:2997/api/game/GetGameFee?pMemberCode=' + memid + '&pHoleTypeID=' + hole + '&pGameDate=' + format($scope.regDate, 'MM/dd/yyyy').toString();
                    ...
                    $scope.payNow = function () {
                        ...
                        var obj = {
                            "Master": {
                                ...
                                "CaddieCode": $scope.cadeinfo ? $scope.cadeinfo : 0,
                                "CaddiePermanent": ($scope.pbcheck ? $scope.pbcheck : 0),
                        ...
                        $http({
                            method: "POST",
                            url: "http://localhost:2997/api/Game/CreateMemberGameExtraPayNew",
                            data: obj
                        }).then(function (response) {
